...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	if (plat->sdiowakeup_irq) {
		...
		if (ret) {
			...
			if(mmc->index ==0)   //SD
				mmc->pm_caps |= MMC_PM_WAKE_SDIO_IRQ;    // 2011-07-13
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
		...
		if(mmc->card && (mmc->card->type != MMC_TYPE_SDIO))
		{
			printk("Wangjin========> %s calls mmc_suspend_host with host named: %s . \n", __func__, mmc_hostname(mmc));
		...
		{			
			if(mmc->card && host->clks_on)
			{
				...
				printk("\n Wangjin==========> %s called to disable clock ! \n", __func__);
				printk("Wangjin========> %s calls mmc_suspend_host with host named: %s . \n", __func__, mmc_hostname(mmc));
			}
		}
		...
		if (!rc) {
			...
			host->sdio_irq_disabled = 1;
			printk("Wangjin========> %s calls with host named: %s . \n", __func__, mmc_hostname(mmc));
			...
			if (mmc->card && (mmc->card->type == MMC_TYPE_SDIO) &&
				...
				if (host->plat->sdiowakeup_irq) 
				{
					printk("Wangjin=======> %s calls enable_irq_wake() \n ", __func__);
...
msmsdcc_runtime_resume(struct device *dev)
{
	...
	if (mmc) {
		...
		spin_unlock_irqrestore(&host->lock, flags);
		if(mmc->card && (mmc->card->type != MMC_TYPE_SDIO))   // SD card
		{
			printk("\n Wangjin========> %s calls mmc_resume_host() with host named: %s . \n", __func__, mmc_hostname(mmc));
			mmc_resume_host(mmc);
		}
		else
		{
			if(mmc->card && !host->clks_on)
			{
				...
				clk_enable(host->clk);				
				host->clks_on = 1;
				printk("\n Wangjin==========> %s called to enable clock ! \n", __func__);
			}
		}
