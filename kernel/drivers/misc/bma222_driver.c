...
#define CMPCOUNT  30
//static short history[3][CMPCOUNT];

static struct timer_list 	refresh_timer;
static atomic_t open_count;
unsigned long	bma_delay = 150;
...
static void refresh_data(unsigned long d )
{
	struct bma222_data *data;

	data = i2c_get_clientdata(bma222_client);

	schedule_work(&data->work_data);
...
static ssize_t bma222_read(struct file *file, char __user * buf, size_t count, loff_t * offset)
{
	...
	ret = bma222_read_accel_xyz(&acc);
	if (ret < 0) {
		printk("XXXXXXXXX bma222 read data error in bma222_read XXXXXXXXX\n");
		return ret;
	}
      /* add cover to aeans project for compass akm8975,send compass algrithom*/
  	coordinate_real_to_android(&acc, BMA222_INSTALL_DIR);
...
static void bma222_work_f(struct work_struct *work)
{
	bma222acc_t acc;
	int ret;
	...
	ret = bma222_read_accel_xyz(&acc);
	if (ret < 0) {
		printk("XXXXXXXXX bma222 read data error in bma222_work_f XXXXXXXXX\n");
		return;
	}
	// convert data to android coordinate system,added by xqj
	coordinate_real_to_android(&acc, BMA222_INSTALL_DIR);
	...
	int i = 0;
	while (i < CMPCOUNT) {
		if (history[0][i] != acc.x || history[1][i] != acc.y || history[2][i] != acc.z)
			break;
		i++;
	}
	if (CMPCOUNT == i) {
		bma222_soft_reset();
		printk("XXXXXXXXXX data error, reset hardware! XXXXXXXXXXXXXX\n");
	}

	for (i = 0; i < CMPCOUNT; i++) {
		history[0][i + 1] = history[0][i];
		history[1][i + 1] = history[1][i];
		history[2][i + 1] = history[2][i];
	}
	history[0][0] = acc.x;
	history[1][0] = acc.y;
	history[2][0] = acc.z;
	...
	msm_sensors_input_report_value(EV_REL, REL_X, acc.x);
	msm_sensors_input_report_value(EV_REL, REL_Y, acc.y);
	msm_sensors_input_report_value(EV_REL, REL_Z, acc.z);
...
static int bma222_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk(KERN_INFO "XXXXXXXXXXXXx%s\n", __FUNCTION__);
	...
	printk(KERN_INFO "XXXXXXXXXXXXXXXXbma222 device create ok\n");
...
/*	err = request_irq(data->IRQ, bma222_irq_handler, IRQF_TRIGGER_RISING, "bma222", &data->bma222);
	...
	INIT_WORK(&data->work_data, bma222_work_f);
	...
	printk("XXXXXXXXXXX BMA222 probe OK XXXXXXXXXXXX\n");
	return 0;
