	...
	int length)
{
   ...
   int ret;
     	for (i = 0; i < length-1; i++)
			if ((ret = (i2c_smbus_write_byte(i2c,txData[i]))) < 0) {
				printk(KERN_ERR "akm8963_i2c_txdata: i2c_smbus_write_byte_data failed   \n");
				return (ret);
			}
...
static void  akm8963_reset(struct akm8963_data *akm)//reset is only  sepecifal for  8963;
{
	...
	//udelay(10);
	int ret;
	ret = gpio_get_value(GPIO_COMPASS_RESET);
	...
	int *rbuf)
{
	...
	if (ready & ORI_DATA_READY) {
		msm_sensors_input_report_value(EV_ABS, ABS_RX, rbuf[0]);
		msm_sensors_input_report_value(EV_ABS, ABS_RY, rbuf[1]);
		msm_sensors_input_report_value(EV_ABS, ABS_RZ, rbuf[2]);
		msm_sensors_input_report_value(EV_ABS, ABS_RUDDER, rbuf[4]);
	...
	if (ready & MAG_DATA_READY) {
		msm_sensors_input_report_value(EV_ABS,ABS_HAT0X, rbuf[9]);
		msm_sensors_input_report_value(EV_ABS, ABS_HAT0Y, rbuf[10]);
		
		msm_sensors_input_report_value(EV_ABS,ABS_BRAKE, rbuf[11]);
		
		msm_sensors_input_report_value(EV_ABS, ABS_GAS, rbuf[4]);
		//	printk(KERN_INFO "akm8963 MAG_DATA_READY\n ");
	}
	...
	struct akm8963_data *akm, char const *buf, size_t count, int pos)
{
	...
	//printk(KERN_INFO "akm8963 delay[%d]=%d\n ",pos,d );
	mutex_unlock(&akm->val_mutex);
...
#endif
static ssize_t akm8963_bin_accel_write(struct file *filp,
	struct kobject *kobj,
	...
	struct input_dev **input)
{
	...
	msm_sensors_input_set_params(EV_ABS, ABS_X,
			-11520, 11520, 0, 0);
	msm_sensors_input_set_params(EV_ABS,ABS_Y,
			-11520, 11520, 0, 0);
	msm_sensors_input_set_params(EV_ABS,ABS_Z,
			-11520, 11520, 0, 0);
	msm_sensors_input_set_params(EV_ABS,ABS_THROTTLE,
			0, 3, 0, 0);
	/* Magnetic field (-4096, 4095)*/
	msm_sensors_input_set_params(EV_ABS, ABS_HAT0X,
			-20480, 20479, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_HAT0Y,
			-20480, 20479, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_BRAKE,
			-20480, 20479, 0, 0);
	msm_sensors_input_set_params(EV_ABS,ABS_GAS,
			0, 3, 0, 0);
	/* Orientation (yaw:0,360 pitch:-180,180 roll:-90,90) */
	msm_sensors_input_set_params(EV_ABS,ABS_RX,
			0, 23040, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_RY,
			-11520, 11520, 0, 0);
	msm_sensors_input_set_params(EV_ABS, ABS_RZ,
			-5760, 5760, 0, 0);
	msm_sensors_input_set_params(EV_ABS,ABS_HAT1Y,
			0, 3, 0, 0);
     msm_sensors_input_set_params(EV_ABS,ABS_RUDDER,
			0, 3, 0, 0);
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
      ...
      if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		printk(KERN_ERR "akm: taos_probe() - i2c smbus byte data functions unsupported\n");
		return -EOPNOTSUPP;
	}
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_WORD_DATA)) {
		printk(KERN_ERR "akm: taos_probe() - i2c smbus word data functions unsupported\n");
	}
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_BLOCK_DATA)) {
		printk(KERN_ERR "akm: taos_probe() - i2c smbus block data functions unsupported\n");
	}
	
      err = gpio_request(GPIO_COMPASS_RESET, "akm8963");
	if (err < 0) {
		 dev_err(&client->dev,
		       "akm8963_probe: request clock gpio failed\n");
		goto exit6;
	}
	err = gpio_direction_output(GPIO_COMPASS_RESET, 1);
	if (err < 0) {
		dev_err(&client->dev,
