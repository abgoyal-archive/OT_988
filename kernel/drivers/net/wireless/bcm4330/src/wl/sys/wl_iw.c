...
extern int chip_is_b1; /* added for bug 151330, make be compatiable for bcm4330 b1 and b2 chip */
...
wl_control_wl_start(struct net_device *dev)
{
	...
	if (g_onoff == G_WLAN_SET_OFF) {
		...
		if(ret)
		{
			WL_ERROR(("sdioh_start called 1 error code=%d in %s \n", ret,  __FUNCTION__));
			goto err;
		}
#endif
		...
		if(ret)
		{
			WL_ERROR(("dhd_dev_reset called error code=%d in %s ! \n", ret, __FUNCTION__));
			goto err;
		}
		//dhd_dev_reset(dev, 0); /* change above line to solve software AP launch problem */
#if defined(BCMLXSDMMC)
		ret = sdioh_start(NULL, 1);
		if(ret)
		{
			WL_ERROR(("sdioh_start called 2  error code=%d  in  %s \n", ret,  __FUNCTION__));
			goto err;
		}
#endif
...
)
{
	...
	if(ret)
	{
		WL_ERROR(("%s called ERROR:===>  failed attempt\n", __FUNCTION__));
		return ret;
	}
#else
	if ((ret = wl_control_wl_start(dev)) != BCME_OK) 
	{
		WL_ERROR(("%s failed first attempt\n", __FUNCTION__));
		
		bcm_mdelay(100);
		
		if ((ret = wl_control_wl_start(dev)) != BCME_OK) 
		{
			WL_ERROR(("%s failed second attempt\n", __FUNCTION__));
...
)
{
	...
	if (g_wps_probe_req_ie) {
		...
		printf(("wl_iw_del_wps_probe_req_ie: is success\n"));
...
)
{
	...
	if (!g_wps_probe_req_ie) {
		...
		//datalen = wrqu->data.length - WPS_ADD_PROBE_REQ_IE_CMD_SUPPLICANT_LENGTH;
		...
		printf(("wl_iw_add_wps_probe_req_ie: is invoked\n"));
		...
		printf(("wl_iw_add_wps_probe_req_ie: is success\n"));
...
#if 1 /* modified for bug 151330, make be compatiable for bcm4330 b1 and b2 chip */
                     WL_SOFTAP(("GOT APSTA FIRMWARE\n")); 
                          
                      if(chip_is_b1) 
                            fwstr = apsta_4330_b1_path; 
                      else 
                            fwstr = apsta_4330_b2_path; 
                                                    
                      ap_fw_loaded = TRUE; 
                      strcpy(fw_path, fwstr); 
                      printk("new fw_path is %s \n", fwstr);  
#else			
			  WL_SOFTAP(("GOT APSTA FIRMWARE\n"));
			  ap_fw_loaded = TRUE;
#endif /* modified end */
...
)
{
	...
	if (dwrq->length && extra) {
		if (strnicmp(extra, "START", strlen("START")) == 0) {
			...
			{
				g_onoff = G_WLAN_SET_OFF;
				WL_ERROR(("Error(ret=%d) occured in %s , while doing START command! \n",  ret, __func__));
			}
			WL_ERROR(("%s, Received regular START command\n", __FUNCTION__));
		}

		if (g_onoff == G_WLAN_SET_OFF) {
			WL_ERROR(("%s, missing START, Fail\n", __FUNCTION__));
			kfree(extra);
			net_os_wake_unlock(dev);
			return -EFAULT;
		}
	    ...
	    if (strnicmp(extra, "SCAN-ACTIVE", strlen("SCAN-ACTIVE")) == 0) {
			...
			WL_ERROR(("%s, #### high level application set country code failed\n", __FUNCTION__));
