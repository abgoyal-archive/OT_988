...
_dhd_sysioc_thread(void *data)
{
	...
	while (down_interruptible(&tsk->sema) == 0) {
		...
		DHD_OS_WAKE_LOCK(&dhd->pub);    //zhbsh
	...
	while (1)
		if (down_interruptible (&tsk->sema) == 0) {
			...
			if (dhd->pub.dongle_reset == FALSE) {
				...
				//zhbsh DHD_OS_WAKE_LOCK(&dhd->pub);
...
dhd_open(struct net_device *net)
{
	...
	if(ret)
	{
		DHD_ERROR(("%s: Error: wl_control_wl_start failed with errcode: %d\n", __FUNCTION__, ret));
		return ret;
	}
	...
	DHD_TRACE(("%s: Enter\n", __FUNCTION__));
        ...
        if ((firmware_path != NULL) && (firmware_path[0] != '\0')) { 
                if(strstr(firmware_path, "test")) {                                             
                        if(chip_is_b1) 
                            strcpy(fw_path, test_4330_b1_path); 
                        else 
                            strcpy(fw_path, test_4330_b2_path); 
                } 
                else { 
                        if(chip_is_b1) 
...
{
/* added for static memory allocation begin */
#if 1
	void *alloc_ptr = NULL;
	extern void *msm_wifi_mem_prealloc(int section, unsigned long size);
	
	alloc_ptr = msm_wifi_mem_prealloc(section, size);
	if (alloc_ptr)
	{
...
#endif
/* added for static memory allocation end */
