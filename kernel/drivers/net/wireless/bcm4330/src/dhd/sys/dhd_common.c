...
dhd_common_init(osl_t *osh)
{
	 ...
	 */
	dhd_msg_level = /*DHD_ERROR_VAL*/0xffff;
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	*/
	...
	if (!ret) {
		...
		bcm_mkiovar("cur_etheraddr", (void *)&ea_addr, ETHER_ADDR_LEN, mac_buf, sizeof(mac_buf));		
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, mac_buf, sizeof(mac_buf), FALSE, 0)) < 0) {
			DHD_ERROR(("%s: can't set MAC address , error=%d ,mac address %02x:%02x:%02x:%02x:%02x:%02x \n", __FUNCTION__, ret ,ea_addr.octet[0], ea_addr.octet[1], ea_addr.octet[2], ea_addr.octet[3], ea_addr.octet[4], ea_addr.octet[5]));
			return BCME_NOTUP;
		}
...
/* disable next lines for browse website via softAP */
#ifdef SOFTAP
	if(!ap_fw_loaded)
#endif /* SOFTAP */	
	{
