...
#ifdef CONFIG_M7LAB_VENUS	// 20110712 FEATURE_M7LAB
...
#define ONCRPC_PM_GET_USB_SERIAL_NUM_PROC  80	// should be larger enough not to be overrid by Qualcomm later

char serial_number[MAX_STR_LEN+1] = "0123456789ABCDEF";
/* String Table */
static struct usb_string strings_dev[] = {
	/* These dummy values should be overridden by platform data */
	[STRING_MANUFACTURER_IDX].s = "Android",
	[STRING_PRODUCT_IDX].s = "Android",
	[STRING_SERIAL_IDX].s = serial_number,
	{  }			/* end of list */
};

// Be sure that serial_number is not null
static int fill_own_usb_serial_num(void)
{
	int i, rc;
	struct msm_rpc_endpoint *serial_num_ep;
	struct get_userial_num_req {
		struct rpc_request_hdr hdr;
		uint32_t serial_num_not_null;
	} req;

	struct get_userial_num_rep {
		struct rpc_reply_hdr hdr;
		uint32_t npi_mode;
		char serial_number[MAX_STR_LEN];
	} rsp;

	serial_num_ep = msm_rpc_connect_compatible(PM_LIBPROG, PM_LIBVERS, 0);
	rc = msm_rpc_call_reply(serial_num_ep, ONCRPC_PM_GET_USB_SERIAL_NUM_PROC,
							&req, sizeof(req),
							&rsp, sizeof(rsp),
							msecs_to_jiffies(5000));

//	Teddy 2011-07-20 Change the sequence
	if (be32_to_cpu(rsp.npi_mode) == 1) {	// to prevent new h/w detection on the manufacturing PC at the factory
		memset(serial_number, 0x0, sizeof(serial_number));
		printk(KERN_WARNING "%s: npi mode. skip usb_serial_number replacement \n", __func__);
		return rc;
	}

	if (rc < 0 || rsp.serial_number[0] == '\0') {
		printk(KERN_ERR "%s: ERROR. msm_rpc_call_reply failed! proc=%d rc=%d\n",
			__func__, ONCRPC_PM_GET_USB_SERIAL_NUM_PROC, rc);
		return rc;
	}

	for (i=0; i<MAX_STR_LEN; i++)
		serial_number[i] = rsp.serial_number[i];
...
static int __init android_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
		...
		{
			for(result=0; result<MAX_STR_LEN; result++)
			{
				serial_number[result] = pdata->serial_number[result];
			}
		}
	}

	fill_own_usb_serial_num();
	//	Teddy 2011-10-07
	if( !strncmp(serial_number, "OT00000000000000", 16) || !strncmp(serial_number, "1234567890ABCDEF", 16) )
	{
		dev->product_id = 0x00F6;
		device_desc.idProduct = __constant_cpu_to_le16(0x00F6);
		if (dev->cdev)
			dev->cdev->desc.idProduct = device_desc.idProduct;
	}
	//	End of Teddy
