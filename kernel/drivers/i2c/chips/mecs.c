	...
	unsigned int cmd, unsigned long arg)
{
	...
	//pr_info("ecompass ecs_ctrl_ioctl leesh cmd = %d\n", cmd);
	...
	case ECOMPASS_IOC_SET_PFLAG:
		//pr_info("here is  ECOMPASS_IOC_SET_PFLAG leesh cmd = %d\n", cmd);
		if (copy_from_user(&flag, pa, sizeof(flag)))
			return -EFAULT;
		if (flag < 0 || flag > 1)
			return -EINVAL;
	...
	case ECOMPASS_IOC_GET_PFLAG:
		//pr_info("here is  ECOMPASS_IOC_GET_PFLAG leesh cmd = %d\n", cmd);
	...
	case ECOMPASS_IOC_GET_OPARMS_ROLL:
	 ...
	 case ECOMPASS_IOC_SET_PPARMS:
		 ...
		 input_set_abs_params(ecs_data_device, ABS_DISTANCE, 
			 parms[0], parms[1], parms[2], parms[3]);
		 break;
	 case ECOMPASS_IOC_GET_PPARMS:
		 break;

	case ECOMPASS_IOC_SET_LPARMS:
		if (copy_from_user(parms, pa, sizeof(parms)))
			return -EFAULT;
		/* orientation roll */
		input_set_abs_params(ecs_data_device, ABS_MISC, 
			parms[0], parms[1], parms[2], parms[3]);
		break;
	case ECOMPASS_IOC_GET_LPARMS:
		break;
	...
	case ECOMPASS_IOC_SET_YPR:
		...
		if (atomic_read(&p_flag)) {
			//pr_info("here is  ECOMPASS_IOC_SET_YPR leesh cmd = %d\n", cmd);
			input_report_abs(ecs_data_device, ABS_DISTANCE, ypr[12]);
			//input_report_abs(ecs_data_device, ABS_TILT_X, ypr[13]);
		}
		else
			input_report_abs(ecs_data_device, ABS_DISTANCE, -1.0f);
		
		if (atomic_read(&l_flag)) {
			//pr_info("here is  ECOMPASS_IOC_SET_YPR leesh cmd = %d\n", cmd);
			input_report_abs(ecs_data_device, ABS_MISC, ypr[13]);
			//input_report_abs(ecs_data_device, ABS_TILT_Y, ypr[15]);
		}
