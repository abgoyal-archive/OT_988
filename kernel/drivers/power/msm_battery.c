...
#ifdef CONFIG_M7LAB_VENUS_BATTERY	//	Teddy 2011-04-05
...
static u32 timeout_timer=(HZ*15);   // 15s timeout
static char boot_first_check = 1;
...
static struct msm_battery_info msm_batt_info = {
	...
	.battery_level = BATTERY_LEVEL_INVALID,	//	Teddy 2011-08-05
...
static char confirm_ac_charger_type = 0;
...
#ifdef CONFIG_M7LAB_VENUS_BATTERY	//	Teddy 2011-02-11
...
{
	static u32 unnecessary_event_count=0;
	static u32 unnecessary_calculate_count = 0;
	u32	charger_status;
	u32	charger_type;
	u32	battery_status;
	u32	battery_level;
...
#ifdef CONFIG_M7LAB_VENUS_BATTERY	//	Teddy 2011-05-31
	...
	DBG_LIMIT("BATT: rcvd: charger_status=%d, charger_type=%d; \n battery_status=%d, battery_level=%d, battery_voltage=%d, battery_temp=%d\n",
	...
	if (msm_batt_info.charger_type != charger_type) {
		unnecessary_calculate_count = 0;
		confirm_ac_charger_type = 0;
...
#ifndef CONFIG_M7LAB_VENUS_BATTERY	//	Teddy 2011-05-30
		if (charger_status == CHARGER_STATUS_INVALID) {
			DBG_LIMIT("BATT: Read VBATT\n");
			battery_voltage = msm_batt_get_vbatt_voltage();
		}
	...
	if (!battery_voltage) {
		...
		{
			battery_voltage = msm_batt_get_vbatt_voltage();
			DBG_LIMIT("-======> at the first time, read Voltage=%d \n",battery_voltage );
		}
	...
	{
		confirm_ac_charger_type++;
		if(confirm_ac_charger_type>2)
			confirm_ac_charger_type = 2;
	}
	else
	{
		confirm_ac_charger_type = 0;
	...
	if(msm_batt_info.batt_capacity == 101)
		unnecessary_calculate_count = 3;
	
	if (msm_batt_info.battery_voltage != battery_voltage) {		
		msm_batt_info.battery_voltage  	= battery_voltage;
		if( (unnecessary_calculate_count > 2) || (msm_batt_info.batt_capacity == 101))
		{
	  ...
	  )
	{
		...
		if(charger_status == 0)
			...
			msm_batt_info.batt_status = POWER_SUPPLY_STATUS_NOT_CHARGING; // POWER_SUPPLY_STATUS_DISCHARGING;
...
#ifdef CONFIG_HAS_EARLYSUSPEND	//	Teddy 2011-06-07
...
#ifdef CONFIG_M7LAB_VENUS_BATTERY	//	Teddy 2011-07-25
...
void msm_batt_late_resume(struct early_suspend *h)
{
	...
	if (msm_batt_info.batt_handle != INVALID_BATT_HANDLE) {
        ...
        if (jrd_start_poll_voltage_timer == 0)
        {
            ...
            mod_timer(&jrd_poll_voltage_timer, jiffies + timeout_timer);
        }
        else
        {
            del_timer(&jrd_poll_voltage_timer);
...
#ifdef CONFIG_M7LAB_VENUS_BATTERY	//	Teddy 2011-07-18
typedef struct
{
	s32 level;
	s32 voltage;
} voltage_to_level_map_type;

static const voltage_to_level_map_type discharge_map[] =
{
	{100,4160},      // 30mV offset
	{99,4060},       // passed
	{98,4040},       // passed
	{97,4030},       // passed
	{96,4010},       // passed
	...
	{8,3430},
	{2,3300},
	{0,3270},        // 30mv offset to 3300mv
};

static const voltage_to_level_map_type ac_charge_map[] =
{
	{100,4200},
	{95,4180},       // passed
	{93,4170},       // passed
...
static const voltage_to_level_map_type usb_charge_map[] =
{
	{100,4200},
	{99,4180},       // passed
	{97,4160},       // passed
	...
	{90,4140},       // passed
	{88,4120},       // passed
	{85,4100},       // passed, switch point, CI---> CV charging
	{79,4090},       // passed, another switch point for 1400mAh
	{73,4070},	     // passed
	{67,4050},       // passed
	{62,4030},	     // passed
	{57,4010},       // passed
	...
	{32,3900},       // ???
	{27,3880},       // passed
	{25,3860},       // 3850mv is one of the usb charging switch point, to get stable voltage.
	{23,3840},       // passed
	{20,3820},       // passed
	{18,3800},       // passed
	{15,3770},       // passed, one switch point for Aeneas
	{10,3680},        // passed
	{5,3630},
	{3,3580},
	{2,3500},
...
static u32 juke_batt_capacity(u32 current_voltage)
{
	const voltage_to_level_map_type *table;
	u32	tablesize=0;
	u32	search_index = 0;
	static u32 old_voltage = BATTERY_HIGH;
	static u32 old_capacity = 200;
	u32	current_capacity = 100;

	if( msm_batt_info.charger_status == CHARGER_STATUS_INVALID )
	{
		table = discharge_map;
		tablesize = sizeof(discharge_map)/sizeof(discharge_map[0]);
		if( (old_voltage < current_voltage ) && (old_voltage + 200 > current_voltage ) )
			current_voltage = old_voltage;
	}
	else
	{
		if (confirm_ac_charger_type>1)
		{
			table = ac_charge_map;
			tablesize = sizeof(ac_charge_map)/sizeof(ac_charge_map[0]);
		}
		else
		{
		    table = usb_charge_map;
			tablesize = sizeof(usb_charge_map)/sizeof(usb_charge_map[0]);
		}
		
		if( !boot_first_check && (old_voltage > current_voltage) && (old_voltage < current_voltage + 300) )
			current_voltage = old_voltage;
	}
	...
	{
		if( table[search_index].voltage < current_voltage )
			break;
		else
			search_index ++;
	}
	...
	if( search_index == 0)
		current_capacity = table[search_index].level;
	else if( search_index == tablesize )
		current_capacity = table[tablesize-1].level;
	else
	{
		current_capacity = ((s32)((table[search_index-1].level - table[search_index].level)
						* ( current_voltage - table[search_index].voltage )
					/ (table[search_index-1].voltage - table[search_index].voltage) )
				+ table[search_index].level);
	}

	if( msm_batt_info.charger_status == CHARGER_STATUS_INVALID )
	{
		if( old_capacity < current_capacity && old_capacity != 200 )
...
static int __devinit msm_batt_probe(struct platform_device *pdev)
{
	...
	if (!msm_batt_info.calculate_capacity)
		msm_batt_info.calculate_capacity = juke_batt_capacity;
#else
	if (!msm_batt_info.calculate_capacity)
...
static void msm_batt_shutdown(struct platform_device *dev)
{
    	del_timer(&jrd_poll_voltage_timer);
    jrd_start_poll_voltage_timer = 0;
	cancel_delayed_work_sync(&jrd_poll_voltage_workqueue);
}
