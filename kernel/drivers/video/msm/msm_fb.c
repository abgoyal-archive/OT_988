...
#define BRIGHTNESS_OFFSET	50 // This value is set according to the framework, this value is the lowest brightness, not Zero.
					...
					enum led_brightness value)
{
	...
	bl_lvl = (2 * value * mfd->panel_info.bl_max)
		/(2 * MAX_BACKLIGHT_BRIGHTNESS);
	/*end by youweiqiang*/
	/* Alvin added for testing */
	printk(KERN_ERR "***************** %s bl_lvl: %d\n", __func__, bl_lvl);
	if(bl_lvl < 3)
		bl_lvl = 3;
	bl_lvl -= 2;
	/* Alvin added for testing */
	...
	if(value != 0)
	{
		if(value < BRIGHTNESS_OFFSET)
				value = BRIGHTNESS_OFFSET;
		
			bl_lvl = (((value - BRIGHTNESS_OFFSET)*(mfd->panel_info.bl_max - 1))
				/(MAX_BACKLIGHT_BRIGHTNESS - BRIGHTNESS_OFFSET)) + 1;
...
#if 0	//	Teddy 2011-07-20 sleep current issue.
#ifdef CONFIG_FB_MSM_OVERLAY
	if(cmd != MSMFB_OVERLAY_UNSET)
#endif
		if(!mfd->op_enable)
			return 0;	// by mark.yi & sinclair.lee 20120203 to fix current issue.    return -EPERM;
#endif
