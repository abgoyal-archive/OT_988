...
#include <mach/clk.h>
#include <linux/wakelock.h>		// 20111214 added by sincliar.lee accept TCT suggestion
...
static struct wake_lock lcdc_wake_lock;	// 20111214 added by sinclair.lee accept TCT suggestion
...
static int lcdc_off(struct platform_device *pdev)
{
	...
	wake_unlock(&lcdc_wake_lock);	// 20111214 added by sinclair.lee accept TCT suggestion
...
static int lcdc_on(struct platform_device *pdev)
{
	...
	if (panel_pixclock_freq > 65000000)
		...
		pm_qos_rate = 65000;
#endif
	wake_lock(&lcdc_wake_lock);	// 20111214 added by sinclair.lee accept TCT suggestion
	...
	if (ret) {
       ...
       //if(lcd_id == 0x01221526) 
       //    ret = clk_set_flags(pixel_mdp_clk, CLKF_INVERT); by youweiqiang
...
static int lcdc_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&lcdc_wake_lock, WAKE_LOCK_IDLE, "lcdc");	// 20111214 added by sinclair.lee accept TCT suggestion
...
lcdc_probe_err:
	wake_lock_destroy(&lcdc_wake_lock);	// 20111214 added by sinclair.lee accept TCT suggestion
