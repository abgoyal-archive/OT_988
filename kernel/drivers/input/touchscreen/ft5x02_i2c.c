...
#define FT5X02_I2C_NAME "ft-touch"
...
static int ft5x02_i2c_rxdata(int txlen, unsigned char *rxdata, int length)
{
	struct i2c_msg msgs[] = {
	{
...
//	printk("=======x = %d, y = %d======\n", 240-touch_point[0].w_tp_x, 320-touch_point[0].w_tp_y);	
		...
		if(touch_point[0].w_tp_y<330){
		

			input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR,255);
			if(touch_point[0].w_tp_y<320){

				if (s_fts_ctp_ver == 0x11)
				{
					input_report_abs(ts->input_dev, ABS_MT_POSITION_X,touch_point[0].w_tp_x);
				}
				else
				{
					input_report_abs(ts->input_dev, ABS_MT_POSITION_X,239-touch_point[0].w_tp_x);
				}
		        	input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,319-touch_point[0].w_tp_y);
			}else{
...
//		}else{
//			 if(touch_point[0].w_tp_x<100){
//				input_report_key(ts->input_dev, KEY_MENU, 1);
//			}else if(touch_point[0].w_tp_x>99&&touch_point[0].w_tp_x<220){
//				input_report_key(ts->input_dev, KEY_SEARCH, 1);
//			}else if(touch_point[0].w_tp_x>219){
	...
	if (ft_ts_data.bt_tp_num >= 2) {
		if(touch_point[1].w_tp_y<330){
			...
			if(touch_point[1].w_tp_y<320){
				...
				{
					input_report_abs(ts->input_dev, ABS_MT_POSITION_X,240-touch_point[1].w_tp_x);

				}
                        input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,320-touch_point[1].w_tp_y);
                        }else{
...
//		else{
//			 if(touch_point[1].w_tp_x<100){
//				input_report_key(ts->input_dev, KEY_MENU, 1);
//			}else if(touch_point[1].w_tp_x>99&&touch_point[1].w_tp_x<220){
//				input_report_key(ts->input_dev, KEY_SEARCH, 1);
//			}else if(touch_point[1].w_tp_x>219){
//				input_report_key(ts->input_dev, KEY_BACK, 1);
//			}
...
int ft5x02_power(int on_off)
{
	...
	if(on_off){
			rc = vreg_set_level(vreg_tp, 2800);
			if (rc) {
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	s_fts_ctp_ver = read_reg(0x2f);
	printk("%s---0x2f-ver:0x%x\n ",__func__,s_fts_ctp_ver);

	s_fts_ctp_ver = read_reg(0x3b);
	printk("%s---TW-ver:0x%x\n ",__func__,s_fts_ctp_ver);
#else

	ret = read_reg(0x3d);
	printk("read ft5x02 id %02x\n", ret);
	if (ret != 0x54) {
	...
	/****************upgrade tp******************/ 
	ret = read_reg(0x3b);
	printk("focaltec's version is 0x%x\n",ret);
...
#if 0    //not support power dower down mode in ft5202 chip
	if (ts->power) {
		ret = ts->power(0);
		if (ret < 0)
			printk(KERN_ERR "ft5x02_ts_resume power off failed\n");
...
static int ft5x02_ts_resume(struct i2c_client *client)
{
	...
	if(ret < 0)
	{	
	    //printk("zhanglu---TW resume write failed\n");	
		return -1;
	}
	if (ts->use_irq)
		enable_irq(client->irq);
