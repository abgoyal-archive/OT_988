...
void PlayerDriver::handleSetDataSource(PlayerSetDataSource* command)
{
    ...
    // begin 285018 add SDP support by Hanling.Zhang @ 20120614
    bool  IsSdpAttachment = false;
    char attachmenttype[16];
    if (strncmp(url, "sharedfd:", strlen("sharedfd:")) == 0)
    {
        FILE *type = fopen("/data/data/com.android.email/files/attche_mimetype", "r");
        if(type == NULL)
        {
            IsSdpAttachment = false;
        } else {
            fgets(attachmenttype, 16, type);
            if (strcasecmp(attachmenttype, "application/sdp") == 0)
            {
                fclose(type);
                type = fopen("/data/data/com.android.email/files/attche_mimetype", "w");
                fclose(type);
                IsSdpAttachment = true;
            } else {
                fclose(type);
                IsSdpAttachment = false;
            }
    ...
    // end 285018 add SDP support by Hanling.Zhang @ 20120614
...
bool PVPlayer::initRender()
{
    // set video surface, if there is one
    if (mSurface != NULL) {
        mPlayerDriver->enqueueCommand(new PlayerSetVideoSurface(mSurface,0,0));
    }
    return true;
}
