...
#if 0 /* disable M7 code for bug 307530 */
#define SET_PRB_REQ	"SET_PRB_REQ "
					       ...
					       size_t ies_len)
{
	...
	wpa_printf(MSG_DEBUG, "%s len = %d", __func__, ies_len);

	probe_req_ie = os_malloc(PROFILE_OFFSET + ies_len);

	if (probe_req_ie == NULL)
		return -1;


	os_memset(probe_req_ie, 0, PROFILE_OFFSET + ies_len);
	os_memset(&iwr, 0, sizeof(iwr));
	os_strncpy(iwr.ifr_name, drv->ifname, IFNAMSIZ);
	os_memcpy(probe_req_ie, SET_PRB_REQ, strlen(SET_PRB_REQ));
	if (ies_len)
		os_memcpy(probe_req_ie+PROFILE_OFFSET, ies, ies_len);
	iwr.u.data.pointer = probe_req_ie;
	iwr.u.data.length = PROFILE_OFFSET + ies_len;
...
const struct wpa_driver_ops wpa_driver_wext_ops = {
	...
	.combo_scan = wpa_driver_wext_combo_scan, //jclee@gtsystem.co.kr
