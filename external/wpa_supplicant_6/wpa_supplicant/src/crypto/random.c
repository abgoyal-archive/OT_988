...
#define ANDROID_BRCM_P2P_PATCH
#ifdef ANDROID_BRCM_P2P_PATCH
static u8 dummy_key[20] = { 0x11, 0xbe, 0x33, 0x43, 0x35, 
...
int random_pool_ready(void)
{
	...
	if (res < 0) {
		...
		wpa_printf(MSG_ERROR, "random: Cannot read from /dev/random: "
			   "%s", strerror(errno));
		res = (sizeof(dummy_key) - dummy_key_avail);		
		for(i=dummy_key_avail; i<sizeof(dummy_key); i++) {
			dummy_key[i] = rand() % 0xff;
		}
#else
		wpa_printf(MSG_ERROR, "random: Cannot read from /dev/random: "
			   "%s", strerror(errno));
