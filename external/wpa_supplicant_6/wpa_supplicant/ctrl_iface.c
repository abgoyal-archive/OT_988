...
//11.08.19, jclee@gtsystem.co.kr, added WPS, refer to chateau [START]
  if(os_strcmp(pin, "") == 0)
  {
  	wpa_printf(MSG_DEBUG, "[ctrl_iface.c][wpa_supplicant_ctrl_iface_wps_pin] pin null");
  	pin = NULL;
  }
//11.08.19, jclee@gtsystem.co.kr, added WPS, refer to chateau [END]
	if (pin) {
...
static char * wpa_supplicant_wps_ie_txt(char *pos, char *end,
					const struct wpa_scan_res *res, struct wpa_supplicant *wpa_s)
{
#ifdef CONFIG_WPS
...
static int wpa_supplicant_ctrl_iface_scan_result(
	const struct wpa_scan_res *res, char *buf, size_t buflen, struct wpa_supplicant *wpa_s)
{
	char *pos, *end;
	int ret;
	const u8 *ie, *ie2;
...
#endif
	pos = wpa_supplicant_wps_ie_txt(pos, end, res, wpa_s);
#ifdef WAPI
					 ...
					 size_t buflen)
{
	...
	if (ie2)
		pos = wpa_supplicant_ie_txt(pos, end, "WPA2", ie2, 2 + ie2[1]);
	pos = wpa_supplicant_wps_ie_txt(pos, end, bss, wpa_s);
